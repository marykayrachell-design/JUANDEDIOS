...
import { Plus, Search, Users, Mail, Phone, X, Edit2, Trash2, CreditCard } from 'lucide-react';
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (editingBeneficiary) {
        const { error } = await supabase.from('beneficiaries').update(formData).eq('id', editingBeneficiary.id);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          beneficiaries.map((beneficiary) => (
            <div key={beneficiary.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all group">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-1">{beneficiary.name}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-900">{editingBeneficiary ? 'Editar Beneficiario' : 'Nuevo Beneficiario'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" className="px-6 py-2 bg-fundacion-blue text-white rounded-xl text-sm font-bold hover:bg-blue-800 shadow-md">
                  {editingBeneficiary ? 'Actualizar' : 'Guardar Beneficiario'}
                </button>
              </div>
            </form>
          </div>
